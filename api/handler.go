...
func GetTimesheet(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := dbConn.Query("SELECT id, company_name, hours_worked, date_worked, incident, resolution, created_at FROM timesheet")
	if err != nil {
	...
	for rows.Next() {
		...
		err := rows.Scan(&timesheet.ID, &timesheet.CompanyName, &timesheet.HoursWorked, &timesheet.DateWorked, &timesheet.Incident, &timesheet.Resolution, &timesheet.CreatedAt)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
...
func CreateTimesheet(w http.ResponseWriter, r *http.Request) {
	...
	result, err := dbConn.Exec("INSERT INTO timesheet (company_name, hours_worked, date_worked, incident, resolution, created_at) VALUES (?, ?, ?, ?)",
		timesheet.CompanyName, timesheet.HoursWorked, dateWorked, timesheet.Incident, timesheet.Resolution, timesheet.CreatedAt)
	if err != nil {
...
func UpdateTimesheet(w http.ResponseWriter, r *http.Request) {
	...
	result, err := dbConn.Exec("UPDATE timesheet SET company_name=?, hours_worked=?, date_worked=?, incident=?, resolution=? WHERE id=?",
		timesheet.CompanyName, timesheet.HoursWorked, dateWorked, timesheet.Incident, timesheet.Resolution, timesheet.ID)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
